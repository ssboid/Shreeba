    ...
    const applyFilters = () => {
      const filtered = goodsData.filter(
        (item) =>
          ...
          (!filters.year || new Date(item.purchasedate).getFullYear().toString() === filters.year)
      );
    ...
    <div className="p-2">
      ...
      <div className="mb-4 mt-4 flex items-center gap-4">
        ...
        >
          ...
          {[...new Set(goodsData.map((item) => item.wholesalername))].map((wholesaler) => (
        ...
        >
          ...
          {[...new Set(goodsData.map((item) => new Date(item.purchasedate).getFullYear().toString()))].map(
            (year) => (
      ...
      {currentPageData.map((item, index) => (
        <div 
          key={index} 
          className="border rounded-md p-4 shadow cursor-pointer hover:shadow-lg transition-shadow"
        ...
        >
          ...
          <p className="text-gray-500">Wholesaler Code: {item.wholesalername}</p>
        </div>
      ))}
    </div>
      ) : (
                  ...
                  <div className="relative">
                    ...
                    {activeActionId === item.id && (
                      <div className="absolute right-0 mt-2 w-40 bg-white border rounded-md shadow-lg z-50">
                        ...
                        <button className="w-full px-4 py-2 text-left hover:bg-red-100 text-red-600"                           onClick={() => handleDeleteClick(item.id)}
                        >
                          Delete
                        </button>
                      </div>
